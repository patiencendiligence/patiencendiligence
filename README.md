...
8개 이상 조직에서 12년간 프론트엔드 개발자로 일하며, 레거시 아키텍처 개선(번들 사이즈 18.6% 감소), 배포 안정화 체계 구축(핫픽스 발생 80% 감소), 개발 자동화 도구 도입(GraphQL codegen, Storybook 기반 컴포넌트 생성 자동화) 등을 진행했습니다. 회사 프로젝트는 코드 공개가 어려워 이 프로필에는 담지 못했습니다 — 자세한 실무 경험은 이력서로 안내드립니다.
